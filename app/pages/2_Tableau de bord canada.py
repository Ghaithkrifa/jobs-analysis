...
import numpy as np
from apicanada import filter_data, canadafiltre, calculer_pourcentage_avancement, localisation, plot_bar_chart, grapheline, plot_line_chart, societe, cambembertchart
...
res = calculer_pourcentage_avancement(df)
...
st.sidebar.image("./images/canada.png", width=290)
st.sidebar.header('Dashboard `Canada`')
st.sidebar.subheader("Visualiser les types d'offre d'emploi")
option_categorie = df["categorie"].unique()
categorie = st.sidebar.selectbox(
    'Choisir une catégorie', option_categorie, key='categorie')

options_sous_categories = df[df['categorie']
...
with c1:
    st.markdown('### Ville')
    filtered_df_ville = df[(df['sous_categorie'] == sous_categorie) & (
...
with c2:
    ...
    filtered_df = df[(df['sous_categorie'] == sous_categorie)
                     & (df['categorie'] == categorie)]
...
with co2:
    ...
    filtered_df_societe = df[(df['sous_categorie'] == sous_categorie) & (
        df['categorie'] == categorie)]
...
ddf = grapheline(categorie_grapheline, sous_categorie_grapheline)
